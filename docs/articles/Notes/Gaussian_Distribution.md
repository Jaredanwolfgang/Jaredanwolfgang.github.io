...
1. 标准差 $\sigma$ 决定了曲线的宽窄，标准差越大，曲线越矮胖；标准差越小，曲线越瘦高
1. 高斯分布的期望值为均值 $\mu$，方差为 $\sigma^2$
1. 高斯分布的协方差矩阵 $\Sigma$ 是对称正定矩阵
1. **高斯分布的边缘分布是高斯分布**（边缘分布是指将多元高斯分布中的某几个维度的变量积分掉后得到的分布）
1. **高斯分布的条件分布是高斯分布**（条件分布是指在已知某些变量的情况下，对另一些变量的分布）
1. **高斯分布的线性变换仍然是高斯分布**（线性变换是指 $y = Ax + b$，其中 $A$ 是矩阵，$b$ 是向量）
:::

高斯分布会在不同的问题中产生，例如：

1. 对于一个一元实值向量，使熵最大的分布是高斯分布
1. 中心极限定理表明，独立同分布的随机变量和服从任意分布的随机变量的和，当样本量足够大时，其分布会趋近于高斯分布
